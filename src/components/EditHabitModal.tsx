...
export default function EditHabitModal({ habit, isOpen, onClose }: EditHabitModalProps) {
  ...
  const handleSave = () => {
    if (!name.trim()) return;
    editHabit(habit.id, { name: name.trim(), emoji, category, target, schedule });
    onClose();
  };
    ...
    >
      <div className="bg-white rounded-t-3xl md:rounded-3xl w-full md:max-w-md p-6 animate-slide-up max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-5">
          ...
          <button onClick={onClose} className="w-9 h-9 flex items-center justify-center rounded-full hover:bg-mint text-muted text-xl cursor-pointer">
            ×
          </button>
        </div>
              ...
              {EMOJI_OPTIONS.map((e) => (
                <button
                  ...
                  className={`text-2xl p-1.5 rounded-lg transition cursor-pointer ${
                    emoji === e ? 'bg-mint ring-2 ring-sage' : 'hover:bg-cream'
                  }`}
                >
                  {e}
                </button>
